...
const Hero = () => {
  ...
  const stories = [
    {
      profilePicture: "http://localhost:5000/uploads/hameed-1748884042411.png",
      name: "Hamid Karimi",
    },
    {
      profilePicture: "http://localhost:5000/uploads/hameed-1748884042411.png",
  ...
  return (
    <div>
      ...
      <div className="w-[75px]">
          ...
          <p className="text-center truncate mt-1 text-gray-800 text-sm">Your Story</p>
        </div>
       ...
       {stories.map((story, index) => (
        <div key={index} className="w-[75px] ">
          <div className="w-[75px] h-[75px] bg-placeholderColor border border-black rounded-2xl overflow-hidden">
            <img className="w-full h-full object-cover" src={story.profilePicture}/>
          </div>
          <p className="truncate mt-1 text-center text-gray-800 text-sm">{story.name}</p>
        </div>
       ))}
      ...
      <div className="bg-placeholderColor w-full h-[203px] overflow-hidden   mt-7 mb-5">
        <img className="w-full h-full object-cover" src="https://i.pinimg.com/736x/90/bb/25/90bb25f9ba49d000185f014a7daee676.jpg"/>
      </div>
    </div>
  );
};
